...
private:
	...
	Vector<double> interior_solution, skeleton_solution, trace_right_hand_side;
	std::vector<double> local_optimal_test_functions, local_bilinear_form_values, intermediate_matrix_storage;
};
...
ConvectionDiffusionDPG<dim>::ConvectionDiffusionDPG ()
                :
				fe_trial_cell (FE_DGQ<dim>(degree), 1, FE_DGQ<dim>(degree), dim), fe_trial_face (FE_FaceQ<dim>(degree + 1), 1, FE_FaceQ<dim>(degree), 1),
                fe_trial (fe_trial_cell, 1, fe_trial_face, 1), fe_test (FE_DGQ<dim>(degree + degree_offset), 1, FE_DGRaviartThomas<dim>(degree + degree_offset - 1), 1),
...
std::vector<double> face_values (no_of_boundary_trial_dofs_per_cell*no_of_test_dofs_per_cell);
...
std::fill(local_bilinear_form_values.begin(), local_bilinear_form_values.end(), 0);
...
template <int dim>
void ConvectionDiffusionDPG<dim>::compute_local_optimal_test_functions (const typename DoFHandler<dim>::active_cell_iterator &test_cell, const FEValues<dim> &fe_values_test_cell, const std::vector<double> &local_bilinear_form_values, std::vector<double> &local_optimal_test_functions)
{
...
// Note: The proposed test norm is NOT good for the hyperbolic problem (epsilon = 0). This is a possible avenue for future research.
    ...
    {
	double CsquareK = fmin(1/vol,1/epsilon);
...
deallog << "Trial degrees of freedom: " << dof_handler_trial.n_dofs() << " (" << dof_handler_trial_cell.n_dofs() << " interior, " << dof_handler_trial_face.n_dofs() << " trace)" << std::endl;
deallog << "Test degrees of freedom: " << dof_handler_test.n_dofs() << std::endl;
std::cout << std::endl;
...
local_optimal_test_functions.resize (no_of_trial_dofs_per_cell*no_of_test_dofs_per_cell, 0);
local_bilinear_form_values.resize (no_of_trial_dofs_per_cell*no_of_test_dofs_per_cell, 0);
intermediate_matrix_storage.resize (no_of_trial_dofs_per_cell*no_of_test_dofs_per_cell*triangulation.n_active_cells(), 0);
}
...
// First we loop over all cells and allocate the values from compute_local_bilinear_form_values to the correct submatrices. We also compute the right-hand side vectors.
...
FullMatrix<double> local_trace_system_matrix (no_of_face_trial_dofs_per_cell, no_of_face_trial_dofs_per_cell);
...
Vector<double> local_trace_right_hand_side (no_of_face_trial_dofs_per_cell);
...
DoFTools::extract_boundary_dofs(dof_handler_trial_face, comp_mask, boundary_dofs);
	...
	local_trace_system_matrix = 0; local_system_submatrix_00 = 0; local_system_submatrix_01 = 0; local_system_submatrix_10 = 0; local_system_submatrix_11 = 0; local_interior_right_hand_side = 0; local_trace_right_hand_side = 0; intermediate_matrix *= 0; intermediate_vector = 0;
        ...
        for (unsigned int i = 0; i < no_of_face_trial_dofs_per_cell; ++i)
	    {
		    ...
		    {
		    trace_system_matrix(local_dof_indices_trial_face[i], local_dof_indices_trial_face[j]) += local_trace_system_matrix(i,j);
		    }

     	trace_right_hand_side(local_dof_indices_trial_face[i]) += local_trace_right_hand_side(i);
	    }
		...
		unsigned int index_no = no_of_interior_trial_dofs_per_cell*no_of_face_trial_dofs_per_cell*cell_no;

		for (unsigned int i = 0; i < no_of_interior_trial_dofs_per_cell; ++i)
		    for (unsigned int j = 0; j < no_of_face_trial_dofs_per_cell; ++j)
			{
			intermediate_matrix_storage[i + j*no_of_interior_trial_dofs_per_cell + index_no] = intermediate_matrix1(i,j);
			}
    }
}
...
{
SolverControl solver_control (10000, 1e-14);
...
std::vector<double> local_interior_solution(no_of_interior_trial_dofs_per_cell);
	...
	std::fill(local_interior_solution.begin(), local_interior_solution.end(), 0);
        ...
        for (unsigned int i = 0; i < no_of_interior_trial_dofs_per_cell; ++i)
		    for (unsigned int j = 0; j < no_of_face_trial_dofs_per_cell; ++j)
		    {
			local_interior_solution[i] += intermediate_matrix_storage[i + j*no_of_interior_trial_dofs_per_cell + index_no]*skeleton_solution(local_dof_indices_trial_face[j]);
		    }

...
double x = 0; double y = 0; double z = 0;
    ...
    values[point] = 2*erf(x/sqrt(2*epsilon))*(epsilon - y*y) + 2*(erf(x/sqrt(2*epsilon)) + x*sqrt(2/(M_PI*epsilon))*exp(-0.5*x*x/epsilon))*(1 - y*y);
	}
}
...
vector(0) = erf(x/sqrt(2*epsilon))*(1 - y*y); // Dirichlet boundary values function enforcing u = g to be applied to all boundary points with indicator 0.
...
return erf(x/sqrt(2*epsilon))*(1 - y*y);
}
};
...
public:
    ...
    const std::string test_norm = "chan"; // choose between the "chan" and "metcalfe" formulation and test norms

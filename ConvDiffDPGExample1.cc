...
double x = 0; double y = 0; double z = 0;
	...
	case 3: values[point][1] = y; values[point][2] = z; break;
    }
	}
}
...
double x = 0; double y = 0; double z = 0;
	...
	double X = (exp((x - 1)/epsilon) - 1)/(exp(-1/epsilon) - 1) + x - 1; double Y = (exp((y - 1)/epsilon) - 1)/(exp(-1/epsilon) - 1) + y - 1;
...
template <int dim> class ExactSolutionValues : public Function<dim>
{
public: virtual double value (const Point<dim> &point, const unsigned int) const override
{
double x = 0; double y = 0; double z = 0;
...
template <int dim> class ExactSolutionXDerivativeValues : public Function<dim>
{
public: virtual double value (const Point<dim> &point, const unsigned int) const override
{
double x = 0; double y = 0; double z = 0;
...
double X = (exp((x - 1)/epsilon))/(epsilon*exp(-1/epsilon) - epsilon) + 1; double Y = (exp((y - 1)/epsilon) - 1)/(exp(-1/epsilon) - 1) + y - 1;
return X*Y;
}
};

template <int dim> class ExactSolutionYDerivativeValues : public Function<dim>
{
public: virtual double value (const Point<dim> &point, const unsigned int) const override
{
double x = 0; double y = 0; double z = 0;
...
double X = (exp((x - 1)/epsilon) - 1)/(exp(-1/epsilon) - 1) + x - 1; double Y = (exp((y - 1)/epsilon))/(epsilon*exp(-1/epsilon) - epsilon) + 1;
return X*Y;
}
};
...
public:
    ...
    const unsigned int degree = 2; // Polynomial degree of the trial space.
	const unsigned int degree_offset = 2; // The amount by which we offset the polynomial degree of the test space.
	const unsigned int no_of_cycles = 40; // The maximum number of solution cycles.
	unsigned int cycle = 1; // The current solution cycle.
	double L2_error = 0; // The L2 error.
	double H1_error = 0; // The H1 error.
    const std::string test_norm = "metcalfe"; // choose between the "chan" and "metcalfe" formulation and test norms
...
private:
	...
	void compute_L2_error (); // Compute the L2 error.
	...
	AffineConstraints<double> trace_constraints;
...
ConvectionDiffusionDPG<dim>::ConvectionDiffusionDPG ()
                :
				fe_trial_interior (FE_DGQ<dim>(degree), 1, FE_DGQ<dim>(degree), dim), fe_trial_trace (FE_TraceQ<dim>(degree + 1), 1, FE_FaceQ<dim>(degree), 1),
...
std::vector<double> face_values (no_of_trace_trial_dofs_per_cell*no_of_test_dofs_per_cell);
    ...
    for (unsigned int quad_point = 0; quad_point < no_of_quad_points_cell; ++quad_point)
		...
		std::vector<double> values(dim+1, 0);
			...
			if (test_norm == "metcalfe") {values[d+1] = (fe_values_test_cell.shape_value_component(k,quad_point,d+1) + epsilon*fe_values_test_cell.shape_grad_component(k,quad_point,0)[d])*fe_values_test_cell.JxW(quad_point);}
			if (test_norm == "chan") {values[d+1] = ((1/epsilon)*fe_values_test_cell.shape_value_component(k,quad_point,d+1) + fe_values_test_cell.shape_grad_component(k,quad_point,0)[d])*fe_values_test_cell.JxW(quad_point);}
		    }
    ...
    for (unsigned int k = 0; k < no_of_test_dofs_per_cell; ++k)
        for (unsigned int l = 0; l < k + 1; ++l)
			...
			double div_tau_k = 0; double div_tau_l = 0; double conv_grad_k = 0; double conv_grad_l = 0;
                ...
                if (test_norm == "metcalfe") {V_basis_matrix_inverse(k,l) += C_K*((1/epsilon)*fe_values_test_cell.shape_value_component(k,quad_point,d+1) + fe_values_test_cell.shape_grad_component(k,quad_point,0)[d])*((1/epsilon)*fe_values_test_cell.shape_value_component(l,quad_point,d+1) + fe_values_test_cell.shape_grad_component(l,quad_point,0)[d])*fe_values_test_cell.JxW(quad_point);}
                if (test_norm == "chan") {V_basis_matrix_inverse(k,l) += C_K*(1/epsilon)*fe_values_test_cell.shape_value_component(k,quad_point,d+1)*fe_values_test_cell.shape_value_component(l,quad_point,d+1)*fe_values_test_cell.JxW(quad_point);}
				}

            if (test_norm == "metcalfe") {V_basis_matrix_inverse(k,l) += (fe_values_test_cell.shape_value_component(k,quad_point,0)*fe_values_test_cell.shape_value_component(l,quad_point,0) 
            ...
            if (test_norm == "chan") {V_basis_matrix_inverse(k,l) += (C_K*fe_values_test_cell.shape_value_component(k,quad_point,0)*fe_values_test_cell.shape_value_component(l,quad_point,0) 
			                                                      + epsilon*fe_values_test_cell.shape_grad_component(k,quad_point,0)*fe_values_test_cell.shape_grad_component(l,quad_point,0)
			                                                      + div_tau_k*div_tau_l + conv_grad_k*conv_grad_l)*fe_values_test_cell.JxW(quad_point);}
            }
...
FEValues<dim> fe_values_trial_cell (fe_trial_interior, quadrature_formula, update_values | update_quadrature_points | update_JxW_values);

const unsigned int no_of_quad_points = quadrature_formula.size(); 
const unsigned int no_of_interior_trial_dofs_per_cell = fe_trial_interior.dofs_per_cell;

std::vector<double> exact_solution_values (no_of_quad_points);
std::vector<double> exact_solution_x_derivative_values (no_of_quad_points);
std::vector<double> exact_solution_y_derivative_values (no_of_quad_points);
std::vector<types::global_dof_index> local_dof_indices_trial_cell (no_of_interior_trial_dofs_per_cell);

typename DoFHandler<dim>::active_cell_iterator trial_cell = dof_handler_trial_interior.begin_active(), final_cell = dof_handler_trial_interior.end();

L2_error = 0; H1_error = 0;

    for (; trial_cell != final_cell; ++trial_cell)
	{
	fe_values_trial_cell.reinit (trial_cell);

	trial_cell->get_dof_indices (local_dof_indices_trial_cell);

	ExactSolutionValues<dim>().value_list (fe_values_trial_cell.get_quadrature_points(), exact_solution_values);
	ExactSolutionXDerivativeValues<dim>().value_list (fe_values_trial_cell.get_quadrature_points(), exact_solution_x_derivative_values);
	ExactSolutionYDerivativeValues<dim>().value_list (fe_values_trial_cell.get_quadrature_points(), exact_solution_y_derivative_values);

	    for (unsigned int quad_point = 0; quad_point < no_of_quad_points; ++quad_point)
		{
		double solution_value = 0; double solution_x_derivative_value = 0; double solution_y_derivative_value = 0;

		    for (unsigned int i = 0; i < no_of_interior_trial_dofs_per_cell; ++i)
			{
			const unsigned int comp_i = fe_trial_interior.system_to_component_index(i).first; 

			if (comp_i == 0) {solution_value += interior_solution(local_dof_indices_trial_cell[i])*fe_values_trial_cell.shape_value_component(i,quad_point,0);}
			if (comp_i == 1) {solution_x_derivative_value += interior_solution(local_dof_indices_trial_cell[i])*fe_values_trial_cell.shape_value_component(i,quad_point,1);}
			if (comp_i == 2) {solution_y_derivative_value += interior_solution(local_dof_indices_trial_cell[i])*fe_values_trial_cell.shape_value_component(i,quad_point,2);}
			}

        L2_error += std::pow(exact_solution_values[quad_point] - solution_value,2)*fe_values_trial_cell.JxW(quad_point);
		H1_error += (std::pow(exact_solution_x_derivative_values[quad_point] - solution_x_derivative_value,2) + std::pow(exact_solution_y_derivative_values[quad_point] - solution_y_derivative_value,2))*fe_values_trial_cell.JxW(quad_point);
		}
	}

L2_error = sqrt(L2_error); H1_error = sqrt(H1_error); H1_error *= epsilon;

std::cout << std::endl << "L2 error: " << L2_error << std::endl << "H1_error: " << H1_error << std::endl << "Error Ratio: " << L2_error/H1_error << std::endl;
}
...
unsigned int cell_no = 0; unsigned int index_no_1 = 0; unsigned int index_no_2 = 0; unsigned int index_no_3 = 0;
	...
	std::fill (v_values.begin(), v_values.end(), 0); std::fill (div_tau_values.begin(), div_tau_values.end(), 0); std::fill (tau_values.begin(), tau_values.end(), 0); std::fill (grad_v_values.begin(), grad_v_values.end(), 0);
		...
		for (unsigned int quad_point = 0; quad_point < no_of_quad_points_cell; ++quad_point)
		{
		if (test_norm == "metcalfe") {refinement_vector(cell_no) += (C_K*grad_v_values[quad_point]*grad_v_values[quad_point] + C_K*(1/epsilon)*(1/epsilon)*tau_values[quad_point]*tau_values[quad_point] + 2*C_K*(1/epsilon)*tau_values[quad_point]*grad_v_values[quad_point]
        ...
        if (test_norm == "chan") {refinement_vector(cell_no) += (C_K*(1/epsilon)*tau_values[quad_point]*tau_values[quad_point] + std::pow(div_tau_values[quad_point],2) + std::pow(convection_values[quad_point]*grad_v_values[quad_point],2) 
									                         + C_K*v_values[quad_point]*v_values[quad_point] + epsilon*grad_v_values[quad_point]*grad_v_values[quad_point])*fe_values_test_cell.JxW(quad_point);}
		}

    refinement_vector(cell_no) = sqrt(refinement_vector(cell_no));
	}

std::cout << "Error Estimator: " << refinement_vector.l2_norm() << std::endl << "Estimator Ratio: " << refinement_vector.l2_norm()/L2_error << std::endl << std::endl;
}
...
{
GridGenerator::hyper_cube (triangulation, 0, 1); triangulation.refine_global (2); // Creates the triangulation and globally refines n times.

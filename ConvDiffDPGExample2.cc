...
double x = 0; double y = 0; double z = 0;
    ...
    switch(dim)
    {
    case 2: values[point][1] = 0; break;
	case 3: values[point][1] = 0; values[point][2] = 0; break;
    }
	}
}
...
if (x == 0) {vector(0) = y*(1-y);} else {vector(0) = 0;} // Dirichlet boundary values function enforcing u = g to be applied to all boundary points with indicator 0.
...
{
double x = point[0]; double y = point[1]; // double z = point[2];
const unsigned int N = 1000; double value = 0;

    for (unsigned int n = 0; n < N; ++n)
	{
	const double lambda = std::pow(2*n*M_PI,2)*epsilon; const double r1 = (1 - sqrt(1 + 4*epsilon*lambda))/(2*epsilon); const double r2 = (1 + sqrt(1 + 4*epsilon*lambda))/(2*epsilon); const double diff = r1 - r2;
	double Cn = 0; switch(n) {case 0: Cn = 1.0/6.0; break; default: Cn = -1.0/(std::pow(n*M_PI,2));}

    const double num = exp(r1*x) - exp(r2*x + diff); const double denom = 1 - exp(diff);

	value += Cn*cos(2*n*M_PI*y)*num/denom;
	}
...
L2_error = 0;
	...
	ExactSolutionValues<dim>().value_list (fe_values_trial_cell.get_quadrature_points(), exact_solution_values);
		...
		double solution_value = 0;
		    ...
		    for (unsigned int i = 0; i < no_of_interior_trial_dofs_per_cell; ++i)
			{
			const unsigned int comp_i = fe_trial_interior.system_to_base_index(i).first.first; 
...
std::cout << "Error Estimator: " << refinement_vector.l2_norm() << std::endl << std::endl;
}
...
GridGenerator::hyper_cube (triangulation, 0, 1); 
    ...
    for (const auto &cell : triangulation.cell_iterators())
        ...
        if ((std::fabs(center(1)) < 1e-12) || (std::fabs(center(1) - 1) < 1e-12)) {face->set_boundary_id(1);}
		}

triangulation.refine_global (2); // Creates the triangulation and globally refines n times.

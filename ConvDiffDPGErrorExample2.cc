...
double epsilon = 0.01; double pi = 3.1415926535897932;
...
{
const unsigned int no_of_points = points.size(); const unsigned int N = 500;
...
double x = 0; double y = 0; double z = 0;
	...
	{
	double ln = 4*n*n*pi*pi*epsilon;
	double r2 = (1 - sqrt(1 + 4*epsilon*ln))/(2*epsilon); double r1 = (1 + sqrt(1 + 4*epsilon*ln))/(2*epsilon); 
	double Cn; if (n == 0) {Cn = 1.0/6.0;} else {Cn = -1.0/(n*n*pi*pi);}

	    for (unsigned int point = 0; point < no_of_points; ++point)
		{
		x = points[point][0]; switch(dim) {case 2: y = points[point][1]; break; case 3: y = points[point][1]; z = points[point][2]; break;}

	    double numer = exp(r2*x) - exp(r1*(x-1) + r2); double denom = 1 - exp(r2 - r1);

		values[point] += Cn*cos(2*n*pi*y)*numer/denom;
		}
	}
}
...
double x = 0; double y = 0; double z = 0;
	...
	double Cn; if (n == 0) {Cn = 1.0/6.0;} else {Cn = -1.0/(n*n*pi*pi);}
	    ...
	    double numer = exp(r2*x) - exp(r1*(x-1) + r2); double denom = 1 - exp(r2 - r1);
		double numerdx = r2*exp(r2*x) - r1*exp(r1*(x-1) + r2);

		gradients[point][0] += Cn*cos(2*n*pi*y)*numerdx/denom;
		gradients[point][1] -= Cn*2*n*pi*sin(2*n*pi*y)*numer/denom;
		}
	}
}
...
public:
	...
	const unsigned int no_of_cycles = 45; // The maximum number of solution cycles.
...
VectorTools::interpolate_boundary_values (dof_handler_trial_trace, 1, DirichletBoundaryValues<dim>(), trace_constraints, dirichlet_comp_mask); // Dirichlet boundary constraints.
VectorTools::interpolate_boundary_values (dof_handler_trial_trace, 2, RobinBoundaryValues<dim>(), trace_constraints, robin_comp_mask); // Robin boundary constraints.
VectorTools::interpolate_boundary_values (dof_handler_trial_trace, 3, RobinBoundaryValues<dim>(), trace_constraints, robin_comp_mask); // Robin boundary constraints.
...
typename DoFHandler<dim>::active_cell_iterator trial_cell_interior = dof_handler_trial_interior.begin_active(), final_cell = dof_handler_trial_interior.end();
...
{
GridGenerator::hyper_cube (triangulation, 0, 1, true); triangulation.refine_global (2); // Creates the triangulation and globally refines n times.

...
template <int dim> void Forcing<dim>::value_list (const std::vector<Point<dim>> &points, std::vector<double> &values, const unsigned int) const
{
const unsigned int no_of_points = points.size();

    for (unsigned int point = 0; point < no_of_points; ++point)
	{
	// const double x = points[point][0]; const double y = points[point][1]; const double z = points[point][2];
...
template <int dim> ConvectionDiffusionDPG<dim>::ConvectionDiffusionDPG ()
                   :
...
template <int dim> void ConvectionDiffusionDPG<dim>::setup_system ()
{
...
std::cout << "Trial degrees of freedom: " << no_of_trial_dofs << " (" << no_of_interior_trial_dofs << " interior, " << no_of_trace_trial_dofs << " trace)" << std::endl;
...
template <int dim> void ConvectionDiffusionDPG<dim>::create_constraints ()
{
...
template <int dim> void ConvectionDiffusionDPG<dim>::assemble_system ()
{
...
template <int dim> void ConvectionDiffusionDPG<dim>::compute_local_optimal_test_functions (const FEValues<dim> &fe_values_test_cell, const std::vector<Tensor<1,dim> > &convection_values, const std::vector<unsigned int> &additional_data, const double &cell_size, std::vector<double> &local_optimal_test_functions)
{
const unsigned int no_of_quad_points_cell = additional_data[0];
const unsigned int no_of_trial_dofs_per_cell = additional_data[2];
const unsigned int no_of_test_dofs_per_cell = additional_data[3];
...
std::vector<types::global_dof_index> local_dof_indices_trial (no_of_trial_dofs_per_cell);
std::vector<types::global_dof_index> local_dof_indices_trial_cell (fe_trial_interior.dofs_per_cell);
std::vector<types::global_dof_index> local_dof_indices_trial_trace (fe_trial_trace.dofs_per_cell);
...
typename DoFHandler<dim>::active_cell_iterator trial_cell_trace = dof_handler_trial_trace.begin_active();
    ...
    for (; trial_cell != final_cell; ++trial_cell, ++trial_cell_interior, ++trial_cell_trace)
	{
	trial_cell->get_dof_indices(local_dof_indices_trial); trial_cell_interior->get_dof_indices(local_dof_indices_trial_cell); trial_cell_trace->get_dof_indices(local_dof_indices_trial_trace); 
...
template <int dim> void ConvectionDiffusionDPG<dim>::compute_error_estimator ()
{
const QGauss<dim>  quadrature_formula_cell (degree+degree_offset+1);
...
template <int dim> void ConvectionDiffusionDPG<dim>::run ()
{
